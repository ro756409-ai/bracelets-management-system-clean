...
export default function WarehouseDashboard() {
  ...
  const { data: products, isLoading } = trpc.employeePortal.productsList.useQuery(undefined, {
    enabled: !!meData,
  });
  const { data: lowStock } = trpc.employeePortal.lowStockProducts.useQuery(undefined, {
    enabled: !!meData,
  });
  ...
  const handleAddMovement = () => {
    ...
    if (isNaN(qty) || qty < 1) { toast.error("الكمية يجب أن تكون 1 على الأقل"); return; }
  ...
  const totalStock = products?.reduce((sum: number, p: any) => sum + (p.currentStock ?? 0), 0) ?? 0;
          ...
          <div className="divide-y">
            {products?.map((product: any) => (
              <div key={product.id} className="p-4 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    variant={product.currentStock <= (product.minStockLevel ?? 15) ? "destructive" : "secondary"}
      ...
      <Dialog open={showHistory} onOpenChange={setShowHistory}>
        <DialogContent className="max-w-lg max-h-[80vh] overflow-y-auto" dir="rtl">
          ...
          <div className="space-y-2">
            {movements && movements.length > 0 ? movements.map((m: any, i: number) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {new Date(m.createdAt).toLocaleDateString('ar-EG', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                  </p>
                </div>
              </div>
            )) : (
