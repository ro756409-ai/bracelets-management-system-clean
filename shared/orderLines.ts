...
/**
 * يفكّ «و» **ضد الكتالوج**: الجزء بيتفصل على «و» لو هو نفسه مش اسم معروف **وواحد على
 * الأقل** من الأجزاء الناتجة اسم معروف؛ غير المعروف بيبقى سطرًا للمراجعة.
 *
...
export function expandSegments(
  ...
  for (const seg of segments) {
    ...
    // فصل لو **واحد على الأقل** من الأجزاء اسم معروف: المعروف يبقى سطرًا محلولًا وغير
    // المعروف سطرًا للمراجعة — بدل ما الجزء كله يتبلع في أول اسم متضمَّن فيه
    // («ذكر التحصين و آية الكرسي والساده» كان بيطلع «آية الكرسي ×3»).
    if (parts.length > 1 && parts.some(p => isKnown(p.text))) {
