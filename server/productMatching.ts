...
export const VARIANT_NAME_ALIASES: string[][] = [
  ...
  ["سادة", "سادا", "ساده"],
  // العميل بيختصر اسم النقش أو بيكتبه بإملاء مختلف — التوسيع نصّي، والمطابقة على كتالوج
  // النشاط الممرَّر بس (لو النشاط ماعندوش «آية الكرسي» مفيش أي مطابقة).
  ["آية الكرسي", "اية الكرسي", "ايه الكرسي", "آيه الكرسي", "ايه كرسي", "اية كرسي", "آية كرسي", "آيه كرسي", "كرسي"],
  ["ذكر التحصين", "ذكر تحصين", "التحصين", "تحصين"],
];

/** «ال» التعريف من كل كلمة: «والساده»→«ساده»، «ايه كرسي» = «ايه الكرسي». */
function stripAl(n: string): string {
  return n.replace(/(^|\s)ال(?=\S)/g, "$1");
}
